...
namespace _9M.Work.Utility
{
    public class XmlHelper
    {
        ...
        public void SetAttributes(string XmlFile, XmlElement xe, Hashtable htAttribute)
        {
        ...
        /// </summary>
        /// <param name="XmlFile">Xml文件路径</param>
        /// <param name="xPath">查询xPath</param>
        /// <returns>返回值</returns>
        public XmlNodeList SelectNodes(string XmlFile, string xPath)
        {
            try
            {
                xmldoc = new XmlDocument();
                xmldoc.Load(XmlFile);
                XmlNodeList xmlnode = xmldoc.SelectNodes(xPath);
                return xmlnode;
            }
            catch (XmlException xe)
            {
