...
namespace _9M.Work.WPF_Common.Converter
{
    public class ValueToProcessConverter : IValueConverter
    {
        ...
        {
            SolidColorBrush colorbrush = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#3FAFDF"));
            //NormalBrush = new SolidColorBrush(Colors.Green);
            //WarnBrush = new SolidColorBrush(Colors.Green);
            NormalBrush = colorbrush;
            WarnBrush = colorbrush;
        ...
        private void DrawingGeometry(DrawingContext drawingContext, double value, double maxValue, double radiusX, double radiusY, double thickness, double padding)
        {
            ...
            else
            {
                ...
                //默认的文本
                FormattedText formatWords = new FormattedText("",
                    CultureInfo.CurrentCulture,
                    FlowDirection.LeftToRight,
                    SuccessRateTypeface,
                    SuccessRateFontSize,
